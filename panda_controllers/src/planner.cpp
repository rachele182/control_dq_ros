...
bool planner_node::init(ros::NodeHandle& node_handle){
  ...
  sub_des_traj_proj_ = node_handle.subscribe(
      "/motion_control_dq/desired_project_trajectory", 1, &planner_node::desiredProjectTrajectoryCallback, this,
      ros::TransportHints().reliable().tcpNoDelay());

  sub_ee_pose = node_handle.subscribe(
      "/motion_control_dq/franka_ee_pose", 1, &planner_node::ee_pose_Callback, this,
      ros::TransportHints().reliable().tcpNoDelay());

  sub_ext_forces = node_handle.subscribe(
      "/motion_control_dq/ext_forces", 1, &planner_node::f_ext_Callback, this,
      ros::TransportHints().reliable().tcpNoDelay());
  

  pub_impedance = node_handle.advertise<panda_controllers::DesiredImpedance>("/motion_control_dq/desired_impedance_project", 1);
